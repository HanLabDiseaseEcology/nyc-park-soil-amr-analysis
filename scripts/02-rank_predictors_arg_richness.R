...
<<<<<<< HEAD:02-rank_predictors_arg_richness.R
...
>>>>>>> WIP-predictor-ranking-analysis:scripts/02-rank_predictors_arg_richness.R
...
# 9 Defining function for fitting single predictor model
fit_one_arg_richness_model <- function(
  ...
  model_formula <- as.formula(
    paste("arg_detection_proportion ~", predictor_name)
  )
...
# 11 Ranking predictors with aicc

model_ranking <- map_dfr(
  model_results,
  "model_summary") %>%
  arrange(aicc) %>%
  mutate(
